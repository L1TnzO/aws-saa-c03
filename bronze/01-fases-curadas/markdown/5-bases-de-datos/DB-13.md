> **BRONZE Markdown** — `DB-13` · Servicio: **DynamoDB DAX** · Fase 5 · Dominios examen: D3
> URL oficial: https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/DAX.html
> Título oficial: In-memory acceleration with DynamoDB Accelerator (DAX)
...
from the primary node to read replicas. For more information, see DAX encryption at rest (./DAXEncryptionAtRest.html).
...
certificate. For more information, see DAX encryption in transit (./DAXEncryptionInTransit.html).

###### Topics

- Use cases for DAX (#DAX.use-cases)
- DAX usage notes (#DAX.usage-notes)
- DAX: How it works (./DAX.concepts.html)
- DAX cluster components (./DAX.concepts.cluster.html)
- Creating a DAX cluster (./DAX.create-cluster.html)
- DAX and DynamoDB consistency models (./DAX.consistency.html)
- Developing with the DynamoDB Accelerator (DAX) client (./DAX.client.html)
- Managing DAX clusters (./DAX.cluster-management.html)
- Monitoring DynamoDB Accelerator (./DAX.Monitoring.html)
- DAX T3/T2 burstable instances (./DAX.Burstable.html)
- DAX access control (./DAX.access-control.html)
- DAX encryption at rest (./DAXEncryptionAtRest.html)
- DAX encryption in transit (./DAXEncryptionInTransit.html)
- Using service-linked IAM roles for DAX (./using-service-linked-roles.html)
- Accessing DAX across AWS accounts (./DAX.cross-account-access.html)
- DAX cluster sizing guide (./DAX.sizing-guide.html)
...
DAX is *not* ideal for the following types of
applications:
